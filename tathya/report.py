...
def render_markdown(findings, asset_dir="assets"):
    ...
    lines.append(f"# Tathya report — {label}")
    lines.append("")
    lines.append(f"> Generated {meta.get('generated_at', '')} by "
                 f"tathya v{meta.get('tool_version', '?')}  ·  "
...
# --- duplicates --------------------------------------------------------
    ...
    lines.append("---")
    lines.append("")
    lines.append("_Generated by tathya. Near-duplicate detection uses a perceptual "
...
def render_html(findings):
    ...
    lines.append(f"<title>Tathya — {_esc(meta.get('dataset_name', 'report'))}</title>")
    lines.append(f"<style>{_CSS}</style></head><body>")
    lines.append('<div class="container">')

    # Hero ------------------------------------------------------------------
    lines.append('<header class="hero">')
    lines.append(f"<h1>Tathya — {_esc(meta.get('dataset_name', 'report'))}</h1>")
    lines.append(f"<p>Full report generated {_esc(meta.get('generated_at', ''))} by "
                 f"tathya v{_esc(meta.get('tool_version', '?'))} · "
...
# Pixel statistics -----------------------------------------------------
    ...
    lines.append("<footer class='foot'>Generated by <strong>tathya</strong> from "
